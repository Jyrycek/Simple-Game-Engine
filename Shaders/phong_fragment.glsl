...
void main(void) {
    ...
    for (int i = 0; i < numberOfLights; ++i) {
        ...
        vec4 totalDiffuse = diff * vec4(lights[i].lightColor * material.diffuse, 1.0) * lights[i].lightIntensity;
        vec3 reflectDir = reflect(-lightDirection, normal);
        float spec = pow(max(dot(ex_viewDirection, reflectDir), 0.0), material.shininess);
        vec4 specular = vec4(lights[i].lightColor * material.specular, 1.0) * spec * lights[i].lightIntensity;
        result += attenuation * (totalDiffuse + specular);
    }
